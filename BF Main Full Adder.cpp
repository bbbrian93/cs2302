...
void Display(vector<bool>answerconversion);

void main()
{
	cout << "Brian Fernandez\n" << "Full Adder\n";
	char continuation; //user input for program continuation
	...
	do {
		...
		cout << input1 << " + " << input2 << " = ";
		Display(Full_adder(boolbinary1, boolbinary2));
	...
	if (check_1 == 1)
	{
		...
		for (int i = 0; i < check1.length(); i++)
		{
			...
			{ 
				if (temp1 != '1')
				{
					cout << check1[i]; // grab binary digit @ index and prints out
				}
			}
		}
	}

	return check_1; // if input does not equal binary numbers, returns a 1
}
int check_input2(string check2)
{
	char temp2;
...
vector<bool>convertbinary1(string conv1) // convert the 1's and 0's to bools
{
	...
	vector<bool> conversion1; //creating a vector to store the conversions
...
vector<bool> Full_adder(vector<bool>binary1, vector<bool>binary2)
{
	...
	if (length1>length2)
	{
		length3 = length1;// this is to get the precise amount # of iterations
		do
		{
